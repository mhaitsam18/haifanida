...
@section('content')
    ...
    <x-card class="lg:w-2/3">
        <form action="/admin/cabang" method="post">
            @csrf
            <x-form-input label="Nama Cabang" name="nama_kantor" :value="old('nama_kantor')" placeholder="Nama Cabang" />
            ...
            <div class="mb-4">
                ...
                <select id="induk_kantor_id" name="induk_kantor_id"
                    ...
                    @foreach ($perwakilans as $perwakilan)
                        <option value="{{ $perwakilan->id }}" @selected($perwakilan->id == old('induk_kantor_id'))>{{ $perwakilan->nama_kantor }}</option>
                    @endforeach
                </select>
