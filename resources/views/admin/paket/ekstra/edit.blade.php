...
@section('content')
    ...
    <x-card class="lg:w-1/2">
        <form action="/admin/paket-ekstra/{{ $paketEkstra->id }}" method="post">
            ...
            <div class="mb-4">
                ...
                <select id="ekstra_id" name="ekstra_id"
                    ...
                    @foreach ($ekstras as $ekstra)
                        <option value="{{ $ekstra->id }}" data-harga="{{ $ekstra->harga_default }}" @selected($ekstra->id == old('ekstra_id', $paketEkstra->ekstra_id))>{{ $ekstra->nama_ekstra }}</option>
                    @endforeach
                </select>
                @error('ekstra_id')
            ...
            <x-form-input label="Harga" name="harga" type="number" :value="old('harga', $paketEkstra->harga)" placeholder="Harga" />

            <div class="flex justify-end gap-2">
                <x-button variant="secondary" :href="'/admin/paket/' . $paketEkstra->paket_id . '/paket-ekstra'">Kembali</x-button>
