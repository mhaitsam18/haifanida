...
@section('content')
    ...
    <x-card class="lg:w-2/3">
        <form action="/admin/pembayaran/{{ $pembayaran->id }}" method="post" enctype="multipart/form-data">
            ...
            <x-form-input label="Jumlah Pembayaran" name="jumlah_pembayaran" type="number" :value="old('jumlah_pembayaran', $pembayaran->jumlah_pembayaran)" placeholder="Jumlah Pembayaran" />
            <x-form-input label="Metode Pembayaran" name="metode_pembayaran" :value="old('metode_pembayaran', $pembayaran->metode_pembayaran)" placeholder="(Transfer / QRIS / dll.)" />
            <x-form-input label="Tanggal Pembayaran" name="tanggal_pembayaran" type="date" :value="old('tanggal_pembayaran', $pembayaran->tanggal_pembayaran ? \Carbon\Carbon::parse($pembayaran->tanggal_pembayaran)->format('Y-m-d') : '')" readonly />
            ...
            <x-form-textarea label="Keterangan" name="keterangan" :value="old('keterangan', $pembayaran->keterangan)" placeholder="Keterangan" />

            <div class="flex justify-end gap-2">
                <x-button variant="secondary" :href="'/admin/' . ($pembayaran->pemesanan_id ? 'pemesanan/' . $pembayaran->pemesanan_id . '/' : '') . 'pembayaran'">Kembali</x-button>
