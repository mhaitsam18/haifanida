...
@section('content')
    ...
    <x-card class="lg:w-1/2">
        <form action="/admin/pemesanan-ekstra/{{ $pemesananEkstra->id }}" method="post">
            @method('put')
            @csrf
            <input type="hidden" name="pemesanan_id" value="{{ $pemesananEkstra->pemesanan_id }}">
            ...
            <div class="mb-4">
                ...
                <select id="ekstra" name="ekstra"
                    ...
                    @foreach ($ekstras as $ekstra)
                        <option value="{{ $ekstra->nama_ekstra }}" data-harga="{{ $ekstra->harga_default }}" data-keterangan="{{ $ekstra->keterangan }}" @selected($ekstra->nama_ekstra == old('ekstra', $pemesananEkstra->ekstra))>
            ...
            <x-form-input label="Jumlah" name="jumlah" type="number" :value="old('jumlah', $pemesananEkstra->jumlah)" placeholder="Jumlah" />
            <p id="keterangan_description" class="-mt-3 mb-4 text-xs text-stone-500"></p>

            <x-form-input label="Total Harga" name="total_harga" type="number" :value="old('total_harga', $pemesananEkstra->total_harga)" placeholder="Total Harga" />
            <x-form-textarea label="Keterangan" name="keterangan" :value="old('keterangan', $pemesananEkstra->keterangan)" placeholder="Keterangan" />

            <div class="flex justify-end gap-2">
                <x-button variant="secondary" :href="'/admin/' . ($pemesananEkstra->pemesanan_id ? 'pemesanan/' . $pemesananEkstra->pemesanan_id . '/' : '') . 'pemesanan-ekstra'">Kembali</x-button>
