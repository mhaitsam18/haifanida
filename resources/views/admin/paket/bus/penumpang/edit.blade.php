...
@section('content')
    ...
    <x-card class="lg:w-1/2">
        <form action="/admin/penumpang/{{ $penumpang->id }}" method="post">
            ...
            <div class="mb-4">
                ...
                <select id="jemaah_id" name="jemaah_id"
                    ...
                    @foreach ($jemaahs as $jemaah)
                        <option value="{{ $jemaah->id }}" @selected(old('jemaah_id', $penumpang->jemaah_id) == $jemaah->id)>{{ $jemaah->nama_lengkap }}</option>
                    @endforeach
                </select>
                @error('jemaah_id')
            ...
            <x-form-input label="Nomor Kursi" name="nomor_kursi" type="number" :value="old('nomor_kursi', $penumpang->nomor_kursi)" placeholder="Nomor Kursi" />

            <div class="flex justify-end gap-2">
                <x-button variant="secondary" :href="'/admin/bus/' . $penumpang->bus_id . '/penumpang'">Kembali</x-button>
