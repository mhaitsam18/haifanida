...
@section('content')
    ...
    <x-card class="lg:w-2/3">
        <form action="/admin/bus-jemaah" method="post" enctype="multipart/form-data">
            ...
            <x-form-input label="Nomor Kursi" name="nomor_kursi" :value="old('nomor_kursi')" placeholder="Nomor Kursi" />

            <div class="flex justify-end gap-2">
                @if ($jemaah)
                    <x-button variant="secondary" :href="'/admin/jemaah/' . $jemaah->id . '/bus'">Kembali</x-button>
                @elseif($bus)
                    <x-button variant="secondary" :href="'/admin/bus/' . $bus->id">Kembali</x-button>
                @endif
