...
@section('content')
    ...
    <div class="grid gap-6 lg:grid-cols-2">
        <x-card>
            <form action="/admin/konten/{{ $konten->id }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                <x-form-input label="Nama Konten" name="nama" :value="old('nama', $konten->nama)" placeholder="Nama" />
                <x-form-input label="Judul" name="judul" :value="old('judul', $konten->judul)" placeholder="Judul" />
                ...
                <x-form-textarea label="Isi Konten" name="isi_konten" :value="old('isi_konten', $konten->isi_konten)" :rows="10" placeholder="Isi Konten" />
        ...
        <x-card title="Pratinjau Gambar">
            <img src="{{ $konten->gambar ? asset('storage/' . $konten->gambar) : '' }}" class="img-preview aspect-4/3 w-full rounded-lg object-cover">
        </x-card>
    </div>
@endsection
