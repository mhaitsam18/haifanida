...
<head>
    ...
    <title>{{ $title }} | Agen Haifa Nida</title>
...
<body class="bg-cream-100 text-stone-800">
    ...
    <header class="bg-maroon-900 text-cream-100">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
            <a href="{{ route('agen.index') }}" class="flex items-center gap-2">
                <img src="/assets/img/logos/logo-lanskap-2.png" alt="Haifa Nida Wisata" class="h-8 w-auto">
                <span class="font-display font-semibold">Portal Agen</span>
            </a>
            <nav class="flex items-center gap-4 text-sm">
                <a href="{{ route('agen.index') }}" class="hover:text-cream-300">Dashboard</a>
                <a href="{{ route('agen.poin.index') }}" class="hover:text-cream-300">Poin Saya</a>
                <form action="{{ route('logout') }}" method="post">
                    @csrf
                    <button type="submit" class="hover:text-cream-300">Keluar</button>
                </form>
            </nav>
        </div>
    </header>
